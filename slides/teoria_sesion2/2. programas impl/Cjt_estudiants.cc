...
void Cjt_estudiants::afegir_estudiant(const Estudiant &est)
{
  ...
  // i és la posició més avançada amb el DNI més petit que dni, si n'hi ha;  
  // si no, i=-1 
  vest[i+1] = est;
  ++nest;
}
...
void Cjt_estudiants::modificar_estudiant(const Estudiant &est)
  /* Pre: existeix un estudiant al paràmetre implícit amb el dni d'est  */
{
...
{
  if (i<1 or i>nest) throw PRO2Excepcio("Index no vàlid per a modificar_iessim");
  vest[i-1] = est;
}
...
{
  if (i<1 or i>nest) throw PRO2Excepcio("Index no vàlid per a consultar_iessim");
  return vest[i-1];
}
