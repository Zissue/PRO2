// en esta version se añaden las excepciones de plantar etc
...
bool cercar (const T& x) const{
/* Pre: cert */
/* Post: el resultat indica si el p.i. conté x */

  return cercar_node(primer_node,x);
}

static bool cercar_node (node_arbreGen* n, const T& x) const{
  bool b;
  if (n==NULL) b=false;
  else {
    if (x==n->info) b=true;
    else{
      int m = n->seg.size(); int i=0;
      b=false; 
      while (i<m and not b) {
	if (cercar_node(n->seg[i],x)) b=true;
	else ++i;
      }
    }
  }
