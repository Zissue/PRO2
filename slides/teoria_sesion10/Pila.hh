...
public:
  ...
  // p2=p1; equivale a p2.operator=(p1); el return es por si queremos
  // hacer p3=p2=p1, que equivale a  p3=p2.operator=(p1);
  ...
  {
    // no se hace nada si la asignacion es tipo p=p;
    if (this != &original) {
      altura = original.altura;
      esborra_node_pila(primer_node);
      primer_node = copia_node_pila(original.primer_node);
       ...
       contingut anterior del p.i. ha estat esborrat */
{
  ...
  // podemos implementar nuevas funciones *de* la clase en fichero aparte

#include "otra_funcion.hh"

  // o directamente en Pila.hh
